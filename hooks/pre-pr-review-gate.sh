...
jq -n '{
  hookSpecificOutput: {
    ...
    permissionDecisionReason: "Pre-PR review gate: this is the first ready-making attempt on this branch, which is held once. This hook cannot see whether a review ran — it stops the first attempt and trusts you for the rest, so the retry succeeding is not evidence that anything was reviewed. If no automated review has covered this diff yet, run ONE review skill now, chosen by the nature of the branch diff: /code-review for implementation-heavy diffs (hunting correctness bugs in code); /risk-review for design/architecture/ops/docs-heavy or wide-blast-radius changes (migrations, auth, payment, deploy/runbook docs). If the diff clearly spans both natures, prefer /risk-review. Address any findings that matter, then retry this exact gh pr command — the gate will not fire again on this branch. If a review already ran, or the user asked to skip it, just retry now."
  }
}'
